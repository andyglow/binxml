 ...
 * <code>BitSet</code> will result in a <code>NullPointerException</code>.  A
 * <code>BitSet</code> is not safe for multithreaded use without external
 * synchronization.
 * </p>
 *
...
public class BitSet implements Cloneable, java.io.Serializable {
    ...
    private final static byte[] trailingZeroTable = {
            -25, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0,
     ...
     * position i % 64 (where bit position 0 refers to the least significant
     * bit and 63 refers to the most significant bit). INVARIANT: The words in
     ...
     *
     * @exception NegativeArraySizeException if the specified initial size is
     *            negative.
     */
     ...
     * WARNING:This function assumes that the number of units actually in use
     * is less than or equal to the current value of unitsInUse!
     */
     ...
     * @return DOCUMENT ME!
     */
    public static BitSet build(long value, int size) {
        BitSet bs = new BitSet(size);

        for (int i = 0; i < size; i++) {
            boolean bit = ((value & (long) Math.pow(2, i)) == 1);
     ...
     * @param size DOCUMENT ME!
     */
    public void set(long value, int offset, int size) {
        int sizeof = sizeOf(value);

        // System.out.println("sizeof: "+value+" = "+sizeof);
        size = (size < sizeof) ? size : sizeof;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((value & mask) == mask) {
                // System.out.println("set "+(offset+i));
                set(offset + i);
            } else {
                // System.out.println("clear "+(offset+i));
                clear(offset + i);
            }
        }
    }
     ...
     * @return DOCUMENT ME!
     */
    public long getPossibleLong(int offset, int length) {
        long res = 0;
        int size = length();
    ...
    public byte[] toByteArray() {
        int bitCount = length();
        int len = (bitCount / 8) + (((bitCount % 8) > 0) ? 1 : 0);
        byte[] tmp = new byte[len];

        for (int i = 0; i < len; i++)
            tmp[i] = (byte) (bits[i / 8] >>> ((i % 4) * 8));
     ...
     * then -1 is returned.  To iterate over the <code>true</code> bits in a
     * <code>BitSet</code>, use the following loop:  for(int
     * i=bs.nextSetBit(0); i>=0; i=bs.nextSetBit(i+1)) { // operate on index i
     * here }
     *
    ...
    /**
     * Returns the "logical size" of this <code>BitSet</code>: the index of the
     * highest set bit in the <code>BitSet</code> plus one. Returns zero if
     * the <code>BitSet</code> contains no set bits.
     *
     ...
     * argument. This bit set is modified so that a bit in it has the value
     * <code>true</code> if and only if one of the following statements holds:
     * 
     * <ul>
     * <li>
     * The bit initially has the value <code>true</code>, and the corresponding
     * bit in the argument has the value <code>false</code>.
     * </li>
     * <li>
     * The bit initially has the value <code>false</code>, and the
     ...
     * array of <code>long</code> integers called, say, <code>bits</code>, in
     * such a manner that bit <code>k</code> is set in the <code>BitSet</code>
     * (for nonnegative values of <code>k</code>) if and only if the
     * expression
     * <pre>
    ...
    /**
     * Cloning this <code>BitSet</code> produces a new <code>BitSet</code> that
    ...
    public String toBinaryString() {
        int numBits = length();
        StringBuffer buffer = new StringBuffer(numBits);
        buffer.append('{');
