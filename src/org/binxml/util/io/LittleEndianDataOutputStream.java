...
public class LittleEndianDataOutputStream extends FilterOutputStream
     ...
     * offset <code>off</code> to the underlying output stream. If no
     ...
     * 
     * <p>
     * The <code>flush</code> method of <code>DataOuputStream</code> calls the
     ...
     * the value <code>(byte)0</code>. If no exception is thrown, the counter
     ...
     * then writes that <code>long</code> value to the underlying output
     * stream as an 8-byte quantity, high byte first. If no exception is
     * thrown, the counter <code>written</code> is incremented by
     * <code>8</code>.
     *
     ...
     * by the <code>writeChar</code> method. If no exception is thrown, the
     ...
     * of the string. Following the length, each character of the string is
     * output, in sequence, using the modified UTF-8 encoding for the
     * character. If no exception is thrown, the counter <code>written</code>
     * is incremented by the total number of bytes written to the output
     * stream. This will be at least two plus the length of <code>str</code>,
     * and at most two plus thrice the length of <code>str</code>.
     * </p>
     *
     ...
     * the modified UTF-8 encoding for the character. If no exception is
     * thrown, the counter <code>written</code> is incremented by the total
     * number of bytes written to the output stream. This will be at least two
     * plus the length of <code>str</code>, and at most two plus thrice the
     ...
     * number of bytes written to this data output stream so far. If the
     * counter overflows, it will be wrapped to Integer.MAX_VALUE.
     *
